...
/**
 * Base URL for remote scene assets hosted on img.ly CDN.
 */
const SCENE_BASE_URL = 'https://img.ly/showcases/cesdk/cases/3d-mockup-editor';
...
/**
 * Get the URL for a product's design scene (from remote CDN).
 */
export function getDesignSceneUrl(productKey: string): string {
  ...
  return `${SCENE_BASE_URL}/${product.assetsFolderName}/design.scene`;
}

/**
 * Get the URL for a product's texture mockup scene (from remote CDN).
 */
export function getMockupSceneUrl(productKey: string): string {
  ...
  return `${SCENE_BASE_URL}/${product.assetsFolderName}/textures/Material_baseColor.scene`;
}
